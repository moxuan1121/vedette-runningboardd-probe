...
writes an append-only text file through `jbroot()`; on-device it therefore
resolves inside RootHide's randomized `.jbroot-*` directory. The package
post-install script pre-creates the same file:
